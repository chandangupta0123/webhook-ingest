...
func TestCacheRecordIsSafeForConcurrentWriters(t *testing.T) {
	c := stats.NewCache()
	...
	if got.CallCount != workers || got.TotalDurationSec != workers*10 {
		t.Fatalf("got %+v, want CallCount=%d TotalDurationSec=%d", got, workers, workers*10)
	}
}
